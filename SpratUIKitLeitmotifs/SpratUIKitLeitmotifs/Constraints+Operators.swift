...
public struct ConstraintFeature {
	...
	public init(item:Any, attribute:NSLayoutAttribute, view:UIView? = nil) {
		self.item = item
		self.attribute = attribute
		self.view = view
	}
	
	private func install(relation:NSLayoutRelation, to constant:CGFloat) {
		guard let lhView:UIView = item as? UIView ?? view else { return }
	...
	public static func ==(lhs:ConstraintFeature, rhs:CGFloat) {
		lhs.install(relation: .equal, to: rhs)
	}
	...
	@discardableResult private func install(relation:NSLayoutRelation, to feature:ConstraintFeature)->NSLayoutConstraint? {
		...
		let constraint:NSLayoutConstraint = NSLayoutConstraint(item: item, attribute: attribute, relatedBy: .equal, toItem: feature.item, attribute: feature.attribute, multiplier: 1.0, constant: 0.0)
	...
	@discardableResult private func install(relation:NSLayoutRelation, to factor:ConstraintFactor)->NSLayoutConstraint? {
		guard let lhView:UIView = item as? UIView ?? view
...
public struct ConstraintFactor {
	...
	public init(feature:ConstraintFeature, factor:CGFloat = 1.0, constant:CGFloat = 0.0, priority:UILayoutPriority = UILayoutPriorityRequired) {
	...
	@discardableResult private func install(relation:NSLayoutRelation, to constant:CGFloat)->NSLayoutConstraint? {
